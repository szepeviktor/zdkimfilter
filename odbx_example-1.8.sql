...
# find the message from a given id, found in the Received: header field
SET @id = '00000000005DC067.000000005ABA664F.00002D28';
SELECT id, date FROM message_in AS m WHERE
m.ino = CONV(SUBSTRING_INDEX(@id, '.', 1), 16, 10) AND
m.mtime = CONV(SUBSTRING_INDEX(SUBSTRING_INDEX(@id, '.', 2), '.', -1), 16, 10) AND
m.pid = CONV(SUBSTRING_INDEX(@id, '.', -1), 16, 10);

# find domains which sent to spamtraps yesterday
SELECT SUM(m.spamtrap) AS traps, d.sent, d.recv, d.domain,
INET_NTOA(CONV(HEX(MIN(m.ip)),16,10)) AS minIP,
INET_NTOA(CONV(HEX(MAX(m.ip)),16,10)) AS maxIP,
FROM_UNIXTIME(MAX(m.mtime)) AS last_trap,
FROM_UNIXTIME(d.last_sent) AS last_sent
FROM domain AS d, msg_ref AS r, message_in AS m
WHERE r.domain = d.id AND r.message_in = m.id AND m.spamtrap > 0 AND
m.mtime < UNIX_TIMESTAMP(CURDATE()) AND
m.mtime >= UNIX_TIMESTAMP(CURDATE() - INTERVAL 1 DAY)
GROUP BY d.id ORDER BY MAX(m.mtime);

# find spammy domains
SELECT AVG(m.score) as av, MIN(m.score), MAX(m.score), d.sent, d.recv,
d.domain, FROM_UNIXTIME(d.last_sent) AS last_sent
FROM domain AS d, msg_ref AS r, message_in AS m
WHERE r.domain = d.id AND r.message_in = m.id AND
m.score IS NOT NULL
GROUP BY d.id ORDER BY av;

# find mailing lists --still needs to be manually vetted
SELECT AVG(m.score) as score, COUNT(*) as cnt, d.sent, d.recv,
d.domain, FROM_UNIXTIME(d.last_sent) AS last_sent
FROM domain AS d, msg_ref AS r, message_in AS m
WHERE r.domain = d.id AND r.message_in = m.id AND
m.mailing_list > 0 AND d.sent > 0 AND d.spamtrap = 0
GROUP BY d.id
HAVING score IS NULL OR score < 10
ORDER BY recv DESC;


# sum yesterday spamtraps into the corresponding domain
UPDATE domain AS d2, (
  SELECT d.id AS id, SUM(m.spamtrap) AS spamtrap
  FROM msg_ref AS r, message_in AS m, domain AS d
  WHERE r.domain = d.id AND r.message_in = m.id AND m.spamtrap > 0 AND
  m.mtime < UNIX_TIMESTAMP(CURDATE()) AND
  m.mtime >= UNIX_TIMESTAMP(CURDATE() - INTERVAL 1 DAY)
...
SET d2.spamtrap = d2.spamtrap + t.spamtrap WHERE d2.id = t.id;

# find if an address is blocked
SELECT COUNT(*) AS blocked FROM dmarc_bounce
WHERE addr = 'user@example.com' AND DATE_ADD(since, INTERVAL days DAY) > NOW();

delimiter //

# Called by the bounce address receiver
# This can be invoked, for example, 
#
# The procedure doubles the days if that is enough for blocking,
# otherwise replaces the record anew.
#
DROP PROCEDURE IF EXISTS block_bounce //
CREATE PROCEDURE block_bounce (
  IN m_addr VARBINARY(320))
  MODIFIES SQL DATA
BEGIN
  DECLARE dummy INT UNSIGNED;
  DECLARE Empty_set CONDITION FOR 1329;
  DECLARE EXIT HANDLER FOR Empty_set
    BEGIN
      REPLACE INTO dmarc_bounce SET addr = m_addr;
    END;
  SELECT days INTO dummy FROM dmarc_bounce WHERE addr = m_addr AND
    DATE_ADD(since, INTERVAL 2*days DAY) > NOW();
  UPDATE dmarc_bounce SET days = 2*days WHERE addr = m_addr LIMIT 1;
END //
