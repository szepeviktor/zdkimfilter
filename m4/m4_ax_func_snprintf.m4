...
if test x$ac_cv_have_working_snprintf$ac_cv_have_working_vsnprintf != "xyesyes"; then
  # not actually using replacement  dnl AC_LIBOBJ(snprintf)
  # dnl AC_MSG_WARN([Replacing missing/broken (v)snprintf() with version from http://www.ijs.si/software/snprintf/.])
  # dnl AC_DEFINE(PREFER_PORTABLE_SNPRINTF, 1, "enable replacement (v)snprintf if system (v)snprintf is broken")
  AC_DEFINE(NON_C99_SPRINTF, 1, "no-op: code is compatible")
fi])
